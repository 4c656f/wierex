...
def response(userid):
    ...
    try:
        response = urllib2.Request("http://127.0.0.1:9001/c4d/", json_encode(data), headers={'Accept': 'application/vnd.travis-ci.2+json'})
        page = urllib2.urlopen(response, context = context)
        page_readed = page.read()
        page.close()
    except:
...
def send_render_info():
    ...
    if response(user_id_file()):
        ...
        #render_estimated_time = jobstatus[0].get('estimated_time')
        #render_time = jobstatus[0].get('render_time')
    else:
        print('Wrong cde')
...
def check_render_process():
    if check_job_status_loop():
        send_render_info()
        threading.Timer(3, check_render_process).start()
    else:
        send_render_info()
        return True
